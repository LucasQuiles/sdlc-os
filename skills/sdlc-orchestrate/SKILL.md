...
2. Dispatch execution units — `sonnet-implementer` runners by default, or tmup-managed Codex lanes when warm context, repo-scale coordination, or interactive reprompting justifies a persistent lane. Parallel when independent. Dispatch `sonnet-implementer` with the Agent tool's `mode: "acceptEdits"` — it writes inside worktree isolation, so edit-permission prompts stall runs without adding safety (Phase 2 permissionMode, enforced at dispatch time).
