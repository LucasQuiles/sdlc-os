...
Every executed check inherits a private runner-owned Unix-socket descriptor. The runner uses bounded canonical `lsof` calls to identify exact holders after leader exit, then revalidates the holder's start identity before signaling it. Linux uses `pidfd` signaling when available; macOS repeats lease and start-identity validation immediately before `kill(2)`. Any monitor timeout, malformed inventory, observed holder, or incomplete cleanup makes the row `VERIFY_BACKGROUND_PROCESS`/`INCONCLUSIVE`. This is leak detection for reviewed commands, not a hostile-process sandbox; committed checks must not deliberately close unknown inherited descriptors before daemonizing.
