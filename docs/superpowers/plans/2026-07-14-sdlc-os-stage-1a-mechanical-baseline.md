...
| `P27-21` | fixture-isolation gap; `MATERIAL`; P0 | Codex root; `S1-02`/T2; before the P27-20 amendment commit | the first correction remapped only quality-budget, while other copied payloads retained `/tmp` and legacy-home absolute paths; AST checks can inspect their directories or execute a discovered tool against those paths | `RESOLVED_IN_PLAN`; 2026-07-14; strictly parse every copied payload, allow only `/tmp/` or one shared dynamically derived legacy-project prefix ending at `/LAB/test-project/`, remap them respectively to `payload-files/scratch/` and `payload-files/project/` beneath `RUNTIME_ROOT`, reject every other absolute/non-string path, require all targets absent, and prove copied bytes contain neither original prefix before execution |
...
- [ ] **Step 1: Write root-resolution and relocated-checkout tests**
  ...
  Run all four relocated scripts with isolated `HOME` and `CLAUDE_PLUGIN_ROOT="$RELOCATED_ROOT"`; set `BENCHMARK_ITERATIONS=1` for the benchmark. Before invoking the copied fixture regression, use canonical Python 3.12 to strictly parse every copied `*.payload.json`. Reject duplicate members, malformed JSON, or a missing/non-string/non-absolute `tool_input.file_path`. Accept only `/tmp/<relative>` or an absolute path containing exactly one `/LAB/test-project/` marker; require every marker-based path to share one dynamically derived prefix, which stays local evidence and is never committed. Remap the former to `$RUNTIME_ROOT/payload-files/scratch/<relative>` and the suffix after the marker to `$RUNTIME_ROOT/payload-files/project/<relative>`, rejecting empty, dot, dot-dot, or escaping segments. Preserve the suffix/basename required by predicates, encode sorted compact JSON, and require every rewritten canonical target to remain absent before and after execution. Before launch, scan the copied payload bytes and require zero `/tmp/` and zero dynamically derived legacy-prefix occurrences. Never inspect, create, replace, or remove any source-declared absolute target. Finish with `rg -n -F '$HOME/LAB/sdlc-os'` over the four executable files and fail on any match.
...
- [ ] **Step 5: Create the Stage 1A manifest and honest baseline inventories**
  ...
  - checkout fixture `0/13`, validator shims `0/14`, dispatcher child path exit `127` with aggregate exit `1`, clone manager `19/24`, clone identifier traversal, both `du -sb` sites, three `mktemp --suffix` sites, tmup stale entry, version/inventory/tsx drift, no-test config, and `P27-01..P27-21` as 1A or plan-review disposition;
