#!/bin/bash
# validate-aqs-artifact.sh — thin shim; logic lives in hooks/validators/aqs-artifact.sh
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "$SCRIPT_DIR/../lib/common.sh"
source "$SCRIPT_DIR/../validators/aqs-artifact.sh"
...
validate_aqs_artifact "$FILE_PATH" "$CONTENT"
exit $?
