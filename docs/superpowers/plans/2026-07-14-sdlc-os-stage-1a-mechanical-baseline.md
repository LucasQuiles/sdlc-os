...
| `P27-04` | verification gap; `MATERIAL`; P0 | Codex root plus non-author reviewers; T8; before release verdict | add written-but-not-consumed mutations, call-removal reachability, DRY/dead-surface fitness, and copy-paste Git-bundle replay | `OWNER_APPROVED_CORRECTION_3`; 2026-07-16; reviewer and usability receipts |
...
| `P27-17` | stream-framing false-pass defect; `CRITICAL`; P0 | Codex root; `F-01`/T1; before exact stderr admission | Bash command substitution removes every trailing newline, so marker-plus-blank-lines can equal a normalized marker string; write the expected one-line marker to a file and compare raw bytes with `cmp -s` | `RESOLVED_IN_PLAN`; 2026-07-14; fixed trailing-blank-line red/green receipt plus full matrix remains mandatory |
| `P27-18` | source/plan default mismatch; `MATERIAL`; P1 | Codex root; `S1-02`/T2; before benchmark mutation | Task 2 says to preserve the existing benchmark count but its example defaults to `100`, while committed source defaults to `10`; retain `10` and allow only explicit `BENCHMARK_ITERATIONS` override | `RESOLVED_IN_PLAN`; 2026-07-14; default/override assertions and relocated one-iteration receipt remain mandatory |
| `P27-19` | red-signature mismatch; `MATERIAL`; P1 | Codex root; `S1-02`/T2; before relocated red | missing child scripts return `127`, but the legacy shim/fixture harnesses record those case failures, suppress child stderr, and return top-level exit `1`; require the observed aggregate exit/count plus a separate raw missing-child probe | `RESOLVED_IN_PLAN`; 2026-07-14; exact aggregate `1` with `0/14` or `0/13`, then exact probe `127` with isolated legacy-path stderr, remains mandatory |
| `P27-20` | release-gate contradiction; `MATERIAL`; P0 | Codex root; `S1-02`/T2 and `S1-07`/T7; before resuming Task 2 green | after root repair the static quality-budget payload dereferences absent `/tmp/quality-budget.yaml`, so the raw suite deterministically reports `12/13`; the plan both assigns fixed-`/tmp` tests to 1C and required `13/13` plus a mandatory raw 1A row | `RESOLVED_IN_PLAN`; 2026-07-14; freeze WIP, remap copied fixture paths into a guaranteed-absent isolated tree, admit only the exact sole `12/13` signature as root-routing proof, remove raw fixture from mandatory 1A rows, and retain a non-green 1C finding whose closure requires isolated `13/13` before Stage 1 exit |
...
| `P27-29` | retained-stream artifact false pass; `MATERIAL`; P0 | Codex root; `S1-02`/T2; before Task 2 source correction | Git/find/rg classifiers tested exit plus empty/exact content without first requiring each captured stdout/stderr path to exist as a regular non-symlink file, so a missing file or a symlink to expected bytes could be admitted as retained evidence | `RESOLVED_IN_PLAN`; 2026-07-14; create captures only beneath a fresh contained result directory, require every stream path to exist as a regular non-symlink file before size/content comparison, and add missing-path plus symlink-to-empty/expected classifier falsifiers |
| `P27-30` | contradictory error-catalog authority; `MATERIAL`; P0 | Codex root; `S1-07`/T7; before the runner commit | the plan required seven labels with no Stage 1A production emitter while also requiring every unused catalog entry to fail, so either implementation path would violate the same gate | `RESOLVED_IN_PLAN`; 2026-07-15; catalog only actual Stage 1A emitters, retain child diagnostics in raw evidence, and represent unavailable Task 8 platform/reviewer evidence as terminal `INCONCLUSIVE` reasons rather than invented error codes |
| `P27-35` | cross-platform verification fixture/runbook conformance; `MATERIAL`; P0 | Codex root; `S1-02`/`S1-04`/`S1-05`/`S1-07`/T8; before a second Linux attempt or reviewer dispatch | frozen candidate `8ebe99ec9c6c9210161562ec5cb38ce53dcff0bd` passed macOS `34/34`, but its first Linux run produced `16 PASS`, `14 FAIL`, and `4 NOT_APPLICABLE`: eleven shell rows inherited a temp base beneath an execute-only ancestor, two Python rows created absent ignored bytecode, the README contract executed a documented macOS assertion on Linux, and four Bridge tests inherited Git's user-configured initial branch | `OWNER_ACCEPTED_CORRECTION_1`; 2026-07-15; preserve attempt-1 receipts, add one falsifying regression per boundary, set the Linux runbook temp base to readable `/tmp`, disable bytecode only on Python manifest rows, keep README parser assertions macOS-specific while replaying with the detected host platform, and initialize test-only Git repositories explicitly on `main`; no validator, Bridge runtime, containment, applicability, or release-threshold change; closure requires focused red/green proof and fresh clean macOS/Linux manifests on the corrected candidate |
| `P27-36` | runbook cannot independently reconstruct a candidate run; `MATERIAL`; P0 | Codex root; `S1-07`/T8; before final reviewer replay | a fresh reviewer could run the manifest only after manually inventing bundle intake, expected-digest, detached-checkout, Node-location, tool-enumeration, and result-validation steps | `OWNER_ACCEPTED_CORRECTION_2`; 2026-07-15; make the README a copy-paste bundle-to-validated-result runbook and execute that path from an immutable bundle |
| `P27-37` | protected environment evidence serializes declared secret-like values; `MATERIAL`; P0 | Codex root; `S1-07`/T7; before candidate refreeze | a benign declared key carrying a secret-like canary yielded `PASS` while raw expanded values were written to `environment.json` | `OWNER_ACCEPTED_CORRECTION_2`; 2026-07-15; retain the real child environment only in memory and persist a tokenized/hash-only replay projection whose canary-absence regression must pass |
| `P27-38` | reparented session descendant can evade process census; `MATERIAL`; P0 | Codex root; `S1-07`/T7; before candidate refreeze | five identical double-fork plus `setsid` attempts returned `PASS` while a closed-stdio survivor remained alive | `OWNER_ACCEPTED_CORRECTION_2`; 2026-07-15; attach a unique inherited Unix-socket ownership handle, enumerate exact holders with canonical `lsof`, terminate only verified holders, and require `VERIFY_BACKGROUND_PROCESS` with no survivor |
...
| Regression | Existing dispatcher exits, shims `14/14`, exact root-routed fixture `12/13` with one named non-green 1C failure, clone total, TypeScript/Vitest; captured legacy fixtures | Pre-change source contract, approved non-policy-change boundary, and explicit prohibition on treating the raw fixture suite as green |
...
Required codes include `F01_FIXTURE_ESCAPE`, `F01_INSTALLED_DRIFT`, `F01_FAILPOINT_NOT_REACHED`, `F01_DRIVER_INTERRUPTED`, `F01_TEMP_TREE_HELPER_MISSING`, `F01_PRE_FAILPOINT_CASES_FAILED`, `F01_RESTORE_FAILED`, `F01_INNER_RETAINED`, `F01_INNER_CONTAINER_RETAINED`, `F01_RESTORE_RETAINED`, `ROOT_INVALID`, `PLUGIN_METADATA_DRIFT`, `INVENTORY_DRIFT`, `VERIFY_MANIFEST_INVALID`, `VERIFY_SELECTION_EMPTY`, `VERIFY_RESULTS_ESCAPE`, `VERIFY_ENV_INVALID`, `VERIFY_PLATFORM_MISMATCH`, `VERIFY_EXECUTABLE_DRIFT`, `VERIFY_EXIT_MISMATCH`, `VERIFY_OBSERVATION_FAILED`, `VERIFY_ENCODING_INVALID`, `VERIFY_OUTPUT_LIMIT`, `VERIFY_WRITE_FAILED`, `VERIFY_TIMEOUT`, `VERIFY_INTERRUPTED`, `VERIFY_ARTIFACT_INVALID`, `VERIFY_BACKGROUND_PROCESS`, `VERIFY_SOURCE_DIRTY`, `VERIFY_SOURCE_MUTATION`, `CANDIDATE_MISMATCH`, and `CLEANUP_FAILED`. `F01_FORCED_ASSERTION_AFTER_VALIDATOR_SWAP` remains a success-path test marker rather than an error code; every other emitted `F01_*` diagnostic is cataloged. Child-specific portable/tmup/tsx/zero-test diagnostics remain hashed raw evidence and are classified by the runner's emitted `VERIFY_*` codes. A missing Task 8 platform or reviewer is a terminal `INCONCLUSIVE` reason, not a Stage 1A production error-code emitter. Tests map every runner failure branch to exactly one code and assert that each production-code emitter resolves to exactly one committed catalog entry with audience, message template, remediation, and evidence fields. The drift scanner excludes test strings and declared selectors/markers; unused catalog entries and emitted-but-undefined codes fail.
...
- [ ] **Step 1: Write root-resolution and relocated-checkout tests**
  ...
  Before the first copied-payload write, also compare no-follow device/inode identities for every source/copy pair and reject equality; a hardlink falsifier must fail before mutation with source bytes unchanged. Resolve Git, find, and rg to absolute executables before use. Create their stdout/stderr captures only at previously absent names beneath a fresh physically contained non-symlink result directory. Before any emptiness, size, hash, or byte comparison, require each capture path to exist as a regular non-symlink file using a no-follow type check; missing, symlinked, directory, special-file, or type-drifted evidence is non-pass. Under `LC_ALL=C`, the non-Git probe runs exactly `"$GIT_BIN" -C "$WORK_ROOT" rev-parse --is-inside-work-tree` and admits only exit `128`, an existing regular non-symlink empty stdout file, and an existing regular non-symlink stderr file containing exactly `fatal: not a git repository (or any of the parent directories): .git` plus one LF; this signature was observed on both required platforms. The empty-directory probe runs exactly `"$FIND_BIN" "$WORK_ROOT" -mindepth 1 -print -quit` and admits only exit `0` with two existing regular non-symlink empty stream files. The literal scan admits only rg exit `1` with two existing regular non-symlink empty stream files. Missing tools and every other exit, stream, or artifact type are failures. Pure classification falsifiers inject Git/find/rg error statuses, wrong Git stderr, missing stream paths, symlinks to empty files, and symlinks to expected Git stderr while proving none can reach PASS.

- [ ] **Step 2: Run the new test and verify it fails for the remaining three scripts**

  Run `bash tests/test-plugin-root-resolution.sh`.

  Expected: explicit/fallback/symlink/invalid helper cases pass. The relocated validator-shim or fixture aggregate exits exactly `1` and reports `0/14` or `0/13`. Because both legacy harnesses suppress or omit the child's missing-path stderr, the new outer relocation test must separately invoke one expected legacy child path under its isolated `HOME`, capture raw streams without a pipeline, and require exact child exit `127`, empty stdout, and stderr containing that isolated `$HOME/LAB/sdlc-os` target. A setup/parser failure, a bare top-level `127`, or an inferred path without the raw probe is not the required behavioral red.
...
- [ ] **Step 3: Adopt the shared resolver without changing assertions**
  ...
  For `test-validator-shims.sh`, derive `HOOKS_DIR="$PLUGIN_ROOT/hooks/scripts"`. For `benchmark-dispatch.sh`, introduce `ITERATIONS="${BENCHMARK_ITERATIONS:-10}"` and replace only the loop bound. Add a source assertion that the unset default remains `10` and the relocated suite's explicit override remains `1`; do not silently expand the historical benchmark duration.
...
- [ ] **Step 4: Prove relocated execution and zero old literals**
  ...
  set -e
  [[ "$relocation_rc" -eq 0 ]]
  [[ -f "$RESULT_DIR/relocation.stdout" && ! -L "$RESULT_DIR/relocation.stdout" ]]
  [[ -f "$RESULT_DIR/relocation.stderr" && ! -L "$RESULT_DIR/relocation.stderr" ]]
  [[ ! -s "$RESULT_DIR/relocation.stderr" ]]
  ...
  set -e
  [[ "$literal_rc" -eq 1 ]]
  [[ -f "$RESULT_DIR/literal-scan.stdout" && ! -L "$RESULT_DIR/literal-scan.stdout" ]]
  [[ -f "$RESULT_DIR/literal-scan.stderr" && ! -L "$RESULT_DIR/literal-scan.stderr" ]]
  [[ ! -s "$RESULT_DIR/literal-scan.stdout" && ! -s "$RESULT_DIR/literal-scan.stderr" ]]
  ```

  Expected: root-resolution suite passes after its retained evidence line decodes to the exact isolated `12/13` fixed-`/tmp` signature and valid remap record; validator shims report `14/14`; dispatcher and one-iteration benchmark pass inside the wrapper; literal scan exits exactly `1` with existing regular non-symlink empty stream files. The wrapper's classifier selftests must reject missing stream paths and symlinks to otherwise admissible empty or exact-content files for Git, find, and rg. Do not invoke the raw source fixture command in 1A because its committed payload names shared `/tmp`; it cannot support 1A or Stage 1 green. A bare outer PASS, missing/symlinked retained stream, or decoded-evidence gap is `INCONCLUSIVE`.
...
- [ ] **Step 1: Write runner contract tests first**
  ...
  - result directories/files use `0700`/`0600`; structured receipts/summaries and protected `environment.json` never contain synthetic secret canaries, exact private environment values, or raw subprocess content; the environment artifact retains only a deterministic tokenized/hash-only replay projection and is excluded from reviewer projections;
...
- [ ] **Step 3: Implement a strict versioned manifest loader**
  ...
  Resolve every path physically beneath the repository root, require a nonexistent root-relative results directory, reject symlink components and unsafe run IDs, and refuse overwrite. Reject shell interpreter command-string flags even though the command is an argv array. Resolve the executable before launch and record its canonical path and fingerprint. Construct the exact minimal child environment described above and expand only the four runner-owned substitutions. Write only a deterministic tokenized/hash-only projection to protected `environment.json`; the receipt records declared tokenized values, a sanitization map, relative artifact path, mode, size, and hash. Never serialize the exact child or ambient environment. Owner-host installed-observation rows explicitly declare `SDLC_INSTALLED_PLUGIN_ROOT=${HOST_HOME}/.claude/plugins/sdlc-os` or `TMUP_PLUGIN_ROOT=${HOST_HOME}/.claude/plugins/tmup`; Linux rows may mark the installed-only observation `NOT_APPLICABLE` with evidence. `inconclusive_exit_codes` is optional, explicit, disjoint from `expected_exit`, and limited to `1..125`.

- [ ] **Step 4: Implement unmasked execution and content-addressed receipts**

  Open the unique result directory with a retained directory descriptor; all raw/temporary/final files use no-follow relative operations against that descriptor, and admission revalidates device/inode/physical ancestry. Use `subprocess.Popen` with the resolved absolute executable, runner-owned PATH, `cwd`, exact constructed `env`, `start_new_session=True`, and a runner-created inherited Unix-socket descriptor, writing to mode-`0600` stdout/stderr handles. Resolve canonical `lsof` before execution. The private socket path is unguessable, mode-contained, outside the repository, and held only by the runner and launched tree; after leader exit, `lsof` identifies its exact holders even across double-fork reparenting and `setsid`. On timeout, interruption, or any holder other than the runner, apply bounded TERM→KILL to the launched group and then only to freshly revalidated socket holders, preserve partial output, require no holder survivor, and classify `INCONCLUSIVE`. The runner is not a hostile-code sandbox: committed command review still forbids deliberate inherited-descriptor closure or daemonization. Evaluate explicit inconclusive exit, true exit, structured observation, check-produced artifacts, post-command source state, and source cleanliness in that order.
...
- [ ] **Step 5: Create the Stage 1A manifest and honest baseline inventories**
  ...
  - checkout fixture `0/13`, validator shims `0/14`, dispatcher child path exit `127` with aggregate exit `1`, clone manager `19/24`, both `du -sb` sites, three `mktemp --suffix` sites, tmup stale entry, version/inventory/tsx drift, no-test config, and `P27-02..P27-30` as 1A or plan-review disposition;
  ...
  Create `verification/error-catalog.json` with schema version, the emitted codes listed under Error Messaging, audience, sanitized template, remediation, evidence fields, and owning requirement. Unit tests scan production implementation surfaces for emitted stable codes and fail on missing, duplicate, or unused catalog entries; child-only diagnostics and Task 8 absence reasons are not padded into the catalog.
...
- [ ] **Step 6: Correct accepted findings within the scoped correction budget**
  ...
  The original correction budget was two cycles. Any additional cycle requires an explicit owner approval naming its finding and boundary. For each accepted 1A finding: add a failing regression, implement the minimum scoped correction, rerun focused tests plus both platform manifests, create a new local commit, and require review of the new candidate. A bridge/prune/inbox/health/service finding is assigned to its owning later release unless it invalidates 1A safety; an invalidating finding stops 1A for a plan amendment.

  Correction cycle 1 is owner-approved for `P27-35` only. Preserve the first Linux result tree and sanitized projection. Before changing correction bytes, prove four exact red conditions: the runbook lacks a Linux command with `TMPDIR=/tmp`; Python manifest rows that can create repository bytecode lack `PYTHONDONTWRITEBYTECODE=1`; the README contract replays its literal macOS platform assertion even when the detected host is Linux; and Bridge test repositories initialize without an explicit `main` branch. Then make only these green changes: add the exact Linux runbook command and prerequisite, declare bytecode suppression on the affected Python rows, substitute the detected host platform only in the temporary executable README replay while retaining the documented macOS parser assertion, and use `git init -b main` in test fixtures. Update `verification/baseline-inventory.json` with `P27-35` and its attempt-1 evidence. Do not change production Bridge/validator behavior, F-01 fixture semantics, manifest platform applicability, runner verdict logic, or any 1B containment surface.

  Correction cycle 2 is the owner-approved final cycle for `P27-36..P27-38`. Preserve the old candidate and all Step 4/Step 5 evidence. First add three executable red proofs: a fresh bundle reviewer cannot follow the README without invented commands; a benign declared environment key containing a secret-like canary appears nowhere in any structured artifact; and a double-forked, reparented `setsid` child with closed stdio cannot return `PASS` or remain alive. Then make only these green changes: add exact bundle digest/intake, detached candidate checkout, enumerated tool/runtime, manifest execution, and result-validation commands; replace raw `environment.json` values with stable literals, substitution tokens, and hashes; and add the inherited Unix-socket ownership monitor with canonical `lsof` holder revalidation and bounded cleanup. Expand the local-`tsx` source mutation to cover its wrapper, update the baseline inventory and runbook prerequisites, and add no containment, bridge, prune, inbox, health, service, or later-stage behavior. Refreeze only after focused red/green proof, the complete unit/static gates, fresh clean macOS/Linux manifests, fresh independent review, fresh adversarial review, and a fresh bundle-runbook replay all bind the same candidate.

  Correction cycle 3 is owner-approved only for the `P27-04` runbook replay failure observed against candidate `b7734edeeffac4f37de6de7dd9c2976fa75640f5`. Preserve that candidate and all prior macOS, Linux, mutation, fitness, independent-review, adversarial-review, and failed replay evidence. First add a deterministic regression that installs a conflicting `tests` package into an isolated Python 3.12 virtual environment and proves the committed metadata unit command cannot be shadowed by ambient site packages; the current `python3.12 -m unittest tests/test_metadata_inventory.py -v` command must fail that regression. Then change only the two manifest unit rows to isolated invocations—`python3.12 -S -m unittest tests/test_metadata_inventory.py -v` and `python3.12 -S -m unittest tests/test_verification_runner.py -v`—and allowlist `-S`. Narrowly extend Python command-shape validation to accept one declared startup flag only before an allowlisted `-m` module invocation; duplicate, misplaced, unknown, and command-string flags remain rejected. Do not change runner verdict logic, unrelated Python rows, or any 1B/1C surface. Update the README's executable contract and `P27-04` inventory record. Refreeze only after the focused red/green proof, complete unit/static gates, fresh clean macOS/Linux manifests, fresh independent review, fresh adversarial review, and a fresh bundle-plus-README copy-paste replay all bind the same new candidate.

  Commit the corrected tests, fixtures, manifest, runbook, and inventory once; then restart Steps 1–3 from the new immutable candidate. The correction closes only when focused regressions pass, macOS again reports every selected row `PASS`, Linux reports every applicable row `PASS` with only evidence-backed `NOT_APPLICABLE` rows, result-tree hashes/modes/candidate bindings validate locally, and no reviewer prompt has been dispatched against a superseded candidate.
...
- [ ] Eight pre-swap cases must be green before the validator move, admitted child stderr is raw-byte-equal to exactly one newline-terminated marker, a trailing blank-line falsifier is rejected, and conclusive direct/concurrent failure takes precedence over exit `3`.
