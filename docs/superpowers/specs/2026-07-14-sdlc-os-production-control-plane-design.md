...
4. Make `.claude-plugin/plugin.json` the version authority at `10.0.0`. Remove the duplicate marketplace version when valid under the plugin schema; otherwise generate and validate it from the manifest. Keep the schema-required top-level marketplace description descriptive only, with no second version authority. A check must fail on future divergence.
...
6. Correct `.claude/CLAUDE.md` to 46 repository agent files and 16 repository skill directories. README's 29-item list must be labeled as the core-agent subset, not the installed total. Runtime-loaded counts are reported separately because runtime composition includes non-repository skills. The prior 30-item claim was a historical heading error: the category rows have enumerated 29 names since their introduction, with no canonical thirtieth member.
