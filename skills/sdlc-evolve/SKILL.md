...
Dispatch `crossmodel-supervisor` in TARGETED mode to review the Evolve changes themselves. The investigator probes the highest-risk domain and the reviewer provides an independent assessment. Use the installed tmup selector policy and live model catalog, record requested and observed models separately, and obtain a new attempt receipt for launch or resume. Reuse trustworthy live context with `harvest -> evaluate -> reprompt`; only replace a lane after terminal failure or an expired lease. This is a required self-modification review: if tmup, the receipt schema, model observation, evidence, or a required role is unavailable, record `REQUIRED_UNAVAILABLE`, retain the Evolve work as incomplete, and escalate rather than skipping. See `sdlc-os:sdlc-crossmodel` for the full adapter lifecycle.
